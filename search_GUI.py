...
class App:
    ...
    def initialize_ui(self):
        ...
        self.selected_place = st.selectbox("Select a popular place in Paris", options=list(self.popular_places_dict.keys()), index=default_index)
    ...
    def get_nearby_restaurants(self):
        ...
        if self.use_spark:
            ...
            st.write(f'### Restaurants found within a radius of :green[{self.radius}] meters around :green[{self.selected_place}]')
            st.write(f'Your position is marked in :red[red], enjoy your meal!')
            self.plot_map_spark(nearby_restaurants)
            st.divider()
            st.write(f'### List of :green[{nearby_restaurants.count()}] restaurants found on the map')
            ...
            if not nearby_restaurants.empty:
                ...
                st.divider()
                st.write(f'### List of :green[{len(nearby_restaurants)}] restaurants found on the map') 
                self.plot_table(nearby_restaurants)
            else:
                st.write("No restaurants found within the specified radius.")

    def plot_map(self, nearby_restaurants):
        """
        ...
        map = folium.Map(location=[self.central_lat, self.central_lon], tiles=tileset, zoom_start=12)
    ...
    def plot_map_spark(self, nearby_restaurants):
        """
        ...
        rows = nearby_restaurants.select('latitude', 'longitude', 'name', 'distance').collect()
    ...
    def plot_table(self, nearby_restaurants):
        """
        ...
        st.dataframe(sorted_nearby_restaurants[['name', 'distance', 'latitude', 'longitude']], height=height, use_container_width=True)


    def plot_table_spark(self, nearby_restaurants):
        # Sort the DataFrame by distance in ascending order
        sorted_nearby_restaurants = nearby_restaurants.orderBy('distance')

        # Collect necessary data
        sorted_nearby_restaurants = sorted_nearby_restaurants.select('name', 'distance', 'latitude', 'longitude').toPandas()
